...
function validateRegisterInput(msg) {
  ...
  if (msg.topic !== 'Quantum Circuit') {
    ...
    (keys.includes('register') && typeof msg.payload.register !== 'number') ||
    keys.includes('qubit')
  ) {
...
function validateQubitsFromSameCircuit(qubits) {
  ...
  let valid = qubits.every(
      (obj) => obj.payload.structure.quantumCircuitId === circuitId,
  );
